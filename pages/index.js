    ...
    <Layout>
      ...
      <Link href="/participants">
        <div className="blue-cover flex justify-center items-center cursor-pointer">
          <h1 className="text-4xl sm:text-5xl text-white text-center font-black absolute z-20">Meet our Participants</h1>
        </div>
      </Link>
...
export async function getStaticProps() {
  ...
  let trainers = files.map(filename => {
    const markdownWithMetadata = fs.readFileSync(`content/trainers/${filename}`).toString();
